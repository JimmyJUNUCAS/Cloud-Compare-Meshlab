...
bool QRanSacSDPlugin::applyFilter(QAction *filter, MeshDocument &md, RichParameterSet & par, vcg::CallBackPos *cb)
{
	...
	std::cout <<"TotalPointCount..."<< s_cloud->size() << std::endl;

	if (s_shapes->size() > 0)
	{
		//std::cout << "shapessize..." << s_shapes->size() << std::endl;
		for (MiscLib::Vector<DetectedShape>::const_iterator it = shapes.begin(); it != shapes.end(); ++it)
		{
			...
			if (shapePointsCount < 0.5*s_cloud->size())
				continue;
			std::cout << "Description..." << desc << std::endl;
			std::cout << "shapePointCount..." << shapePointsCount << std::endl;
			...
			case 1: //sphere
			{
				...
				std::cout <<"Center"<< CC.getValue() << std::endl;
